...
namespace KorobiShop.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public IEnumerable<Product> GetAllDiscountProduct()
        {
            ...
            try
            {
                return _context.product.FromSqlRaw($"SELECT ISNULL(p.id , 0) as id, ISNULL(title , '') as name, ISNULL(sd.discount_Amount , 0) as price, ISNULL(sd.oldPrice , 0) as salePrice, " +
            ...
            "p RIGHT JOIN  [korobi_shop_maindatabase].[korobi_shop_admin].[product_category] c ON p.catagory_id = c.id  RIGHT JOIN [korobi_shop_maindatabase].[korobi_shop_admin].[display_specialSales] sd on sd.product_id = p.id where sd.display = 1 and sd.catagory_id=1001 order by sd.dsiplayRank asc").ToList();
